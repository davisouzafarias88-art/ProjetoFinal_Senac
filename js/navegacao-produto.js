...
function navegarParaProduto(nomeProduto) {
    // Detectar se está em nav2 ou layout principal
    const isNav2 = window.location.pathname.includes('/nav2.html/');
    const url = isNav2 
        ? `../../html/produto.html?produto=${encodeURIComponent(nomeProduto)}`
        : `produto.html?produto=${encodeURIComponent(nomeProduto)}`;
